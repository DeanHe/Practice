...
2 Use a recursive search dfs(start, picked, prod, path) that picks the next divisor from divs[start...], multiplies it into prod,
and appends to path until you have k factors whose product is n.
3 During the search, keep track of the best path that minimizes max(path) – min(path) and return it.

Analysis:
DFS
prune with making sure products array is in ascending order
"""
from typing import List


class BalancedKFactorDecomposition:
    def minDifference(self, n: int, k: int) -> List[int]:
        ...
        def dfs(target, start, cnt, products):
            if cnt == k:
                if divisors[start] <= target:
            ...
            for i in range(start, len(divisors)):
                if target % divisors[i] == 0:
                    products.append(divisors[i])
                    dfs(target // divisors[i], i, cnt + 1, products)
                    products.pop()
