...
class Robot:
    def __init__(self, position, health, direction, idx):
...
class RobotCollisions:
    def survivedRobotsHealths(self, positions: List[int], healths: List[int], directions: str) -> List[int]:
        ...
        robots = []
        for i in range(n):
            robots.append(Robot(positions[i], healths[i], directions[i], i))
        # sort by positions
        ...
        for robot in robots:
            if robot.direction == 'R':
                ...
                self.collide(stack, robot)
        # sort by idx
        stack.sort(key=lambda x: x.idx)
        res_health = []
        for robot in stack:
            res_health.append(robot.health)
        return res_health

    def collide(self, stack, robot):
        # robot win
        while stack and stack[-1].direction == 'R' and stack[-1].health < robot.health:
            stack.pop()
            robot.health -= 1
        if stack and stack[-1].direction == 'R':
            # robot draw
