...
Why is the final answer f[1] instead of f[0]?
The reason is that Alice cannot choose index 0 on her first move.
Therefore, the first valid index she can choose is 1, making f[1] the required answer.
"""
...
class StoneGameVIII:
    def stoneGameVIII(self, stones: List[int]) -> int:
        ...
        for i in range(len(stones) - 2, -1, -1):
            dp[i] = max(dp[i + 1], pre_sum[i] - dp[i + 1])
        return dp[1]