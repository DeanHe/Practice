...
class MaximizeTheNumberOfTargetNodesAfterConnectingTreesI:
    def maxTargetNodes(self, edges1: List[List[int]], edges2: List[List[int]], k: int) -> List[int]:
        ...
        def cnt_of_nodes_dist_less_than_k(cur, pre, graph, max_dist):
            if max_dist == 0:
                return 1
            cnt = 1
            for nb in graph[cur]:
                if nb != pre:
                    cnt += cnt_of_nodes_dist_less_than_k(nb, cur, graph, max_dist - 1)
            return cnt
        ...
        res = [0] * g1_nodes
        most2 = 0
        for i in range(g2_nodes):
            most2 = max(most2, cnt_of_nodes_dist_less_than_k(i, -1, g2, k - 1))
        for i in range(g1_nodes):
            res[i] = most2 + cnt_of_nodes_dist_less_than_k(i, -1, g1, k)
        return res
