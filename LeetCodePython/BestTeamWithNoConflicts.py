...
analysis:
dp[i] means the max score with player[i] included
TC: O(NlogN) + O(N^2)
"""
...
class BestTeamWithNoConflicts:
    def bestTeamScore(self, scores: List[int], ages: List[int]) -> int:
        res, n = 0, len(scores)
        dp = [0] * n
        players = sorted(list(zip(ages, scores)), key=lambda player: (player[0], player[1]))
        for i in range(n):
            dp[i] = players[i][1]
            for j in range(i):
                if players[j][1] <= players[i][1]:  # score
