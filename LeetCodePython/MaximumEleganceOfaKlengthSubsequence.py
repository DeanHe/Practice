...
class Solution:
    def findMaximumElegance(self, items: List[List[int]], k: int) -> int:
        ...
        res = cur = 0
        duplicate_category_profit = []
        unique_category = set()
        for i, (profit, category) in enumerate(items):
            if i < k:
                if category in unique_category:
                    duplicate_category_profit.append(profit)
                cur += profit
            else:
                if category not in unique_category:
                    if not duplicate_category_profit:
                        break
                    cur += profit - duplicate_category_profit.pop()
            unique_category.add(category)
            res = max(res, cur + len(unique_category) * len(unique_category))
        return res