...
class MinimumCostForCuttingCakeII:
    def minimumCost(self, m: int, n: int, horizontalCut: List[int], verticalCut: List[int]) -> int:
        ...
        horizontalCut.sort()
        verticalCut.sort()
        horizontal_cut_sum = sum(horizontalCut)
        vertical_cut_sum = sum(verticalCut)
        while horizontalCut and verticalCut:
            if horizontalCut[-1] < verticalCut[-1]:
                res += verticalCut[-1] + horizontal_cut_sum
                vertical_cut_sum -= verticalCut.pop()
            else:
                res += horizontalCut[-1] + vertical_cut_sum
                horizontal_cut_sum -= horizontalCut.pop()
        return res + horizontal_cut_sum + vertical_cut_sum
