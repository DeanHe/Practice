...
class MaximumAveragePassRatio:
    def maxAverageRatio(self, classes: List[List[int]], extraStudents: int) -> float:
        ...
        def improvement(numerator, denominator):
            return (numerator + 1) / (denominator + 1) - numerator / denominator

        pq = []
        for passes, total in classes:
            heapq.heappush(pq, (-improvement(passes, total), passes, total))
        for _ in range(extraStudents):
            _, passes, total = heapq.heappop(pq)
            heapq.heappush(pq, (-improvement(passes + 1, total + 1), passes + 1, total + 1))
        while pq:
            _, passes, total = heapq.heappop(pq)
            res += passes / total
        return res / sz
