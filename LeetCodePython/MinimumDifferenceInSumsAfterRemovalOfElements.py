...
part1[i] means minimum total of subsequence [:i] with n // 3 elements
TC:O(NlogN)
"""
import heapq
...
class MinimumDifferenceInSumsAfterRemovalOfElements:
    def minimumDifference(self, nums: List[int]) -> int:
        ...
        total = sum(nums[: sz // 3])
        # max heap
        pq = [-x for x in nums[: sz // 3]]
        heapq.heapify(pq)
        part1[0] = total

        for i in range(sz // 3, sz * 2 // 3):
            total += nums[i]
            heapq.heappush(pq, -nums[i])
            total -= -heapq.heappop(pq)
            part1[i - sz // 3 + 1] = total

        part2 = sum(nums[sz * 2 // 3 :])
        pq = nums[sz * 2 // 3 :]
        # min heap
        heapq.heapify(pq)
        res = part1[sz // 3] - part2

        for i in range(sz * 2 // 3 - 1, sz // 3 - 1, -1):
            part2 += nums[i]
            heapq.heappush(pq, nums[i])
            part2 -= heapq.heappop(pq)
            res = min(res, part1[i - sz // 3] - part2)
        return res