...
class MeetingRoomsIII:
    def mostBooked(self, n: int, meetings: List[List[int]]) -> int:
        cnt, res, most = [0] * n, 0, 0
        meetings.sort()
        free_rooms, occupied_rooms = [], []
        for i in range(n):
            heapq.heappush(free_rooms, i)
        for s, e in meetings:
            while occupied_rooms and occupied_rooms[0][0] <= s:
                _, occupied_room_id = heapq.heappop(occupied_rooms)
                heapq.heappush(free_rooms, occupied_room_id)
            if free_rooms:
                ...
                cnt[occupied_room_id] += 1
                heapq.heappush(occupied_rooms, (available + e - s, occupied_room_id))
