...
public class CrackingTheSafe {
	public String crackSafe(int n, int k) {
		...
		findPwd(pwd, visited, totalComb, n, k);
		return pwd.toString();
	}

	private boolean findPwd(StringBuilder pwd, Set<String> visited, int totalComb, int n, int k) {
		...
		for (int i = 0; i < k; i++) {
			...
			if (!visited.contains(temp)) {
				visited.add(temp);
				pwd.append(c);
				if(findPwd(pwd, visited, totalComb, n, k)){
					return true;
				}
