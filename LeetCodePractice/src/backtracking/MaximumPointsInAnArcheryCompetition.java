...
public class MaximumPointsInAnArcheryCompetition {
    ...
    public int[] maximumBobPoints(int numArrows, int[] aliceArrows) {
        dfs(numArrows, aliceArrows, new int[aliceArrows.length], 0, 0);
        return res;
    }

    private void dfs(int numArrows, int[] aliceArrows, int[] tmp, int sum, int idx) {
        ...
        for (int i = idx; i < aliceArrows.length; i++) {
            if (numArrows >= aliceArrows[i] + 1) {
                tmp[i] = aliceArrows[i] + 1;
                dfs(numArrows - aliceArrows[i] - 1, aliceArrows, tmp, sum + i, i + 1);
                tmp[i] = 0;
            } else {
                dfs(numArrows, aliceArrows, tmp, sum, i + 1);
            }
        }
    }
}
