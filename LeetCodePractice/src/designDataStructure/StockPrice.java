...
public class StockPrice {
    TreeMap<Integer, Integer> timePrice, priceCount;
    ...
    public void update(int timestamp, int price) {
        if (timePrice.containsKey(timestamp)) {
            ...
            if (priceCount.containsKey(prePrice)) {
                int cnt = priceCount.get(prePrice);
                if (cnt == 1) {
                    priceCount.remove(prePrice);
                } else {
                    priceCount.put(prePrice, cnt - 1);
                }
            }
        }
