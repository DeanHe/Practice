...
public class MovieRentingSystem {
    ...
    HashMap<Integer, Set<Entry>> unrented = new HashMap<>();
    HashMap<Integer, Map<Integer, Integer>> items = new HashMap<>();
    ...
    public MovieRentingSystem(int n, int[][] entries) {
        for(int[] entry : entries){
            ...
            int movie = entry[1];
            int price = entry[2];
            unrented.computeIfAbsent(movie, x -> new TreeSet<>(comparator))
                    .add(new Entry(price, shop, movie));
            items.computeIfAbsent(shop, x -> new HashMap<>())
                    .put(movie, price);
        }
    }
    ...
    public List<Integer> search(int movie) {
        return unrented.getOrDefault(movie, Collections.emptySet())
                .stream().limit(5).map(x -> x.shop).collect(Collectors.toList());
    }

    public void rent(int shop, int movie) {
        int price = items.get(shop).get(movie);
    ...
    public List<List<Integer>> report() {
        return rented.stream().limit(5)
                .map(x -> List.of(x.shop, x.movie)).collect(Collectors.toList());
    }
