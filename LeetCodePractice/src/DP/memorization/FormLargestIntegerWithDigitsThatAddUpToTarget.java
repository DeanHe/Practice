...
public class FormLargestIntegerWithDigitsThatAddUpToTarget {
    ...
    public String largestNumber(int[] cost, int target) {
        mem = new String[target + 1]; // mem[i] means largest number of cost i
        return dfs(cost, target);
    }

    private String dfs(int[] cost, int target){
        if(target == 0){
            return "";
        }
        if(mem[target] != null){
        ...
        for(int d = 1; d <= 9; d++){
            if(target >= cost[d - 1]){
                String temp = dfs(cost, target - cost[d - 1]);
                if(temp != "0"){
                    temp = d + temp;
                    if(res.equals("0")
                            || temp.length() > res.length()
                            || temp.length() == res.length() && res.compareTo(temp) < 0){ //skip unnecessary compare
                        res = temp;
                    }
                }
            }
