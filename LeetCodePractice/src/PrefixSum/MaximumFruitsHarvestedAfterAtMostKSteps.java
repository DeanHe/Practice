...
public class MaximumFruitsHarvestedAfterAtMostKSteps {
    public int maxTotalFruits(int[][] fruits, int startPos, int k) {
        int maxLen = (int) (2e5 + 2);
        int res = 0, l, r;
        int[] preSum = new int[maxLen];
        for (int[] f : fruits) {
            preSum[f[0] + 1] = f[1];
        }
        ...
        startPos++; // incremented startPos by one to make preSum calculations easier.
        for (r = startPos; r < maxLen && r <= startPos + k; r++) {
            l = Math.min(startPos, startPos - (k - 2 * (r - startPos)));
            l = Math.max(1, l);
            res = Math.max(res, preSum[r] - preSum[l - 1]);
        }
        for (l = startPos; l > 0 && l >= startPos - k; l--) {
            r = Math.max(startPos, startPos + (k - 2 * (startPos - l)));
            r = Math.min(maxLen - 1, r);
            res = Math.max(res, preSum[r] - preSum[l - 1]);
        }
        return res;
    }
