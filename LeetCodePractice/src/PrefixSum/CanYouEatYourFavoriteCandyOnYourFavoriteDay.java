...
attn: days are 0-indexed add 1 to day
 */
public class CanYouEatYourFavoriteCandyOnYourFavoriteDay {
    public boolean[] canEat(int[] candiesCount, int[][] queries) {
        ...
        for(int i = 0; i < res.length; i++){
            res[i] = helper(preSum, queries[i]);
        }
        return res;
    }

    private boolean helper(long[] preSum, int[] query){
        int type = query[0];
        int days = query[1] + 1;
        long cap = query[2];
        return days * cap > preSum[type] && days <= preSum[type + 1];
    }
}
