...
public class MaximumSideLengthOfaSquareWithSumLessThanOrEqualToThreshold {
    ...
    private boolean isSquareExist(int[][] sum, int thres, int len) {
        for (int r = 1; r + len <= rows; r++) {
            for (int c = 1; c + len <= cols; c++) {
                int cur = sum[r + len][c + len] - sum[r + len][c] - sum[r][c + len] + sum[r][c];
                if (cur <= thres) {
                    return true;
                }
            }
