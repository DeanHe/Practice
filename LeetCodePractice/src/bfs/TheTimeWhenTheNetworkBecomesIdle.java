...
analysis:
BFS; min dist from 0 to all nodes
round trip time
 */
public class TheTimeWhenTheNetworkBecomesIdle {
    public int networkBecomesIdle(int[][] edges, int[] patience) {
        ...
        for(int i = 1; i < n; i++){
            int rtt = dist[i] * 2 - 1;
            int lastStart = patience[i] * (rtt / patience[i]);
            res = Math.max(res, lastStart + dist[i] * 2);
        }
        return res + 1;
    }

    private void bfs(Map<Integer, List<Integer>> g, int n, int[] dist) {
