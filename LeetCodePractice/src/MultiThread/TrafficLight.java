...
public class TrafficLight {
    ...
    private AtomicBoolean greenOnReadA;
    private ReentrantReadWriteLock greenSignalLock;
    public TrafficLight() {
        this.greenOnReadA = new AtomicBoolean(true);
    ...
    public void carArrived(
        ...
        if(!isGreen(roadId)) {
            this.greenSignalLock.readLock().unlock();
            this.greenSignalLock.writeLock().lock();

            if (!isGreen(roadId)) {
                // change signal to green
                this.greenOnReadA.set(!this.greenOnReadA.get());
                turnGreen.run();
            }

            this.greenSignalLock.readLock().lock();
            this.greenSignalLock.writeLock().unlock();
        }
