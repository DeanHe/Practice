...
public class MaximumVacationDays {
    ...
    public int maxVacationDays(int[][] flights, int[][] days) {
        ...
        int res = 0;
        Integer[][] dp = new Integer[weeks + 1][cities + 1];
        dp[0][0] = 0;
        for(int w = 0; w < weeks; w++){
            for(int c = 0; c < cities; c++){
                int temp = Integer.MIN_VALUE;
                for(int nb = 0; nb < cities; nb++){
                    if(flights[nb][c] == 1 && dp[w][nb] != null){
                        temp = Math.max(temp, dp[w][nb]);
                    } else if(nb == c && dp[w][c] != null){
                        temp = Math.max(temp, dp[w][c]);
                    }
                }
                if(temp != Integer.MIN_VALUE){
                    dp[w + 1][c] = temp + days[c][w];
                }
            }
        }
        for(Integer sum : dp[weeks]){
            if(sum != null){
