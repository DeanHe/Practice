...
public class MaximumProfitOfOperatingaCentennialWheel {
    public int minOperationsMaxProfit(int[] customers, int boardingCost, int runningCost) {
        int maxProfit = 0, idx = 0, remainPeople = 0, round = 0, onboardPeople = 0;
        remainPeople += customers[0];
        while (remainPeople > 0 || round < customers.length){
            round++;
            if(remainPeople > 4){
                onboardPeople += 4;
                remainPeople -= 4;
            } else {
                onboardPeople += remainPeople;
                remainPeople = 0;
            }
            int profit = onboardPeople * boardingCost - round * runningCost;
            if(profit > maxProfit){
                maxProfit = profit;
                idx = round;
            }
            if(round < customers.length){
                remainPeople += customers[round];
            }
        }
        return maxProfit > 0 ? idx : -1;
    }
}
