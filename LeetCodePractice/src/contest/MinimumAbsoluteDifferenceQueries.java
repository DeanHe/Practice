...
analysis: prefix sum
create freq array with size equals to nums max val
create dp matrix with size equals 1: nums.length 2: nums max val
 */
public class MinimumAbsoluteDifferenceQueries {
    public int[] minDifference(int[] nums, int[][] queries) {
        int[] res = new int[queries.length];
        int[][] dp = new int[nums.length + 1][101];
        ...
        for (int i = 0; i < queries.length; i++) {
            ...
            res[i] = minAbs != Integer.MAX_VALUE ? minAbs : -1;
        }
        return res;
    }
}
