...
public class ConstructStringWithRepeatLimit {
    public String repeatLimitedString(String s, int repeatLimit) {
        ...
        int lastCnt = 0;
        while(!cnt.isEmpty()) {
            char last = cnt.lastKey();
            if(sb.length() == 0 || lastCnt < repeatLimit || last != sb.charAt(sb.length() - 1)){
                if(sb.length() > 0 && last != sb.charAt(sb.length() - 1)){
                    lastCnt = 0;
                }
                sb.append(cnt.lastKey());
                if(cnt.get(last) > 0){
                ...
                lastCnt = 0;
                if(cnt.lowerKey(last) == null){
                    break;
                }
                char secondLast = cnt.lowerKey(last);
                sb.append(secondLast);
                if(cnt.get(secondLast) > 0){
                    cnt.put(secondLast, cnt.get(secondLast) - 1);
                    if(cnt.get(secondLast) == 0){
                        cnt.remove(secondLast);
                    }
                }
                lastCnt++;
            }
